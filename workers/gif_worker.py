"""
Worker thread for GIF generation (runs in background to keep UI responsive).
"""
...
class GifWorker(QThread):
    progress = Signal(int)
    finished = Signal(int)   # output file size in bytes
    error = Signal(str)

    def __init__(self, video_path: str, start_sec: float, end_sec: float,
                 output_path: str, text_config: dict, gif_config: dict):
        super().__init__()
    ...
    def run(self):
        try:
            # Create independent VideoProcessor for this thread
            vp = VideoProcessor(self.video_path)
            orig_fps = vp.fps
            target_fps = orig_fps * self.gif_config["fps_factor"]
            frame_skip = self.gif_config["frame_skip"]

            frame_gen = vp.extract_frames(self.start_sec, self.end_sec,
                                          target_fps, frame_skip)
